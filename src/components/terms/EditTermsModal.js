...
const EditTermsModal = ({ open, onClose, text, onSave }) => {
  const [newText, setNewText] = useState(text);

  if (!open) return null;

  const handleSave = () => {
    if (!newText.trim()) return alert("Terms text cannot be empty.");
    onSave(newText);
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-lg rounded-xl shadow-xl flex flex-col max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-5 flex justify-between items-center border-b">
            <h2 className="text-lg font-semibold">Edit Terms & Conditions</h2>
            <button
              onClick={onClose}
